...
// ─── Mission Card (Accordion + Stempelkarte) ──────────────────────
    ...
    <Card style={{ marginBottom: "8px", padding: 0, overflow: "hidden" }}>
      ...
      {open && (
        <div style={{ borderTop: `1px solid ${C.greyBg}`, padding: "14px", background: `${C.greyBg}55` }}>
          ...
          {!done && started && <button onClick={stamp} disabled={stamping} style={{ width: "100%", padding: "12px", background: stamping ? C.greyBg : C.green, border: "none", borderRadius: "12px", color: stamping ? C.textLight : C.white, fontSize: "14px", fontWeight: "700", transition: "all 0.2s" }}>{stamping ? "..." : "Stempel eintragen ✓"}</button>}
...
const AdminPanel = ({ onClose }) => {
  ...
  const saveMission = async () => {
    ...
    try {
      if (editMission.id) {
        const { error } = await supabase.from('missions').update({ title: editMission.title, description: editMission.description, pts_reward: parseInt(editMission.pts_reward) || 0, icon: editMission.icon, goal: parseInt(editMission.goal) || 1, active: editMission.active !== false }).eq('id', editMission.id);
        if (error) { ok2("Fehler: " + error.message, false); return; }
      } else {
        const { error } = await supabase.from('missions').insert({ title: editMission.title || 'Neue Mission', description: editMission.description || '', pts_reward: parseInt(editMission.pts_reward) || 100, icon: editMission.icon || '★', goal: parseInt(editMission.goal) || 1, active: true });
        if (error) { ok2("Fehler: " + error.message, false); return; }
      }
